...
| 012 | Prop projections have never seen recent form | P1 | M | 010, 013, 014 | **Tasks 1-3 DONE 2026-09-17** (`adb66d8`, `033d712`, `6c8c62e`): dead kwarg and dead pre-game fetch removed; recent form bounded to before the game (lookahead guard); **one edge formula** — the `abs(diff/line)*100` fallback is gone, so a star means one thing. **Task 4 blocked on data**: only 2 games have box scores, so 0 players have the 3 logs now required. |
